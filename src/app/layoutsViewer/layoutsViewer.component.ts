...
export class LayoutsViewerComponent implements OnInit {
    ...
    this._route.params.subscribe((params) => {
      this._layoutService.loadPicture(params['id']).subscribe((imgItem) => {
        ...
        if (!this.imgItem?.imgDataURI) {
          this._layoutService
            .loadImgAsBase64(this.imgItem.url)
            .subscribe((image) => {
  ...
  public bindBox(boxEditor) {
    this.meta.boxEditor = boxEditor;
  }
  public bindHintBox(hintBox) {
    this.meta.hintBox = hintBox;
  }

  public cutBoxAndDownload() {
    if (!this.meta.boxEditor.getBox().isVisible) {
      return;
    }

    let canvas: any = document.createElement('canvas');
    let sizeX =
      this.meta.boxEditor.getBox().right - this.meta.boxEditor.getBox().left;
    let sizeY =
      this.meta.boxEditor.getBox().bottom - this.meta.boxEditor.getBox().top;
    canvas.width = sizeX;
    canvas.height = sizeY;

    let pixelData = this.meta.canvas
      .getContext('2d')
      .getImageData(
    ...
    this._layoutService
      .getAndIncLayoutViewerCropFileCounter(this.imgItem.id)
      .subscribe((counter) => {
        let filename = this.imgItem.name + '_' + counter + '.png';
        let imgDataUrl = canvas
          .toDataURL('image/png')
  ...
  public handleMouseScrollEvents(event: WheelEvent) {
    if (this.meta.verticalScroll) {
      ...
      this.meta.left -= this.meta.pixelScroll
        ? this.sign(event.deltaX)
        : Math.round(event.deltaX);
      if (this.meta.left < -this.imgItem.width) {
        this.meta.left = -this.imgItem.width;
      }
      if (this.meta.left > this.meta.windowWidth) {
        this.meta.left = this.meta.windowWidth;
      }
    } else {
      this.meta.top -= this.meta.pixelScroll
        ? this.sign(event.deltaY)
  ...
  public centerPicture() {
    this.meta.left = Math.floor(
      (this.meta.windowWidth - this.imgItem.width) / 2
    );
    if (this.imgItem.height < this.meta.windowHeight) {
      this.meta.top = Math.floor(
        (this.meta.windowHeight - this.imgItem.height) / 2
      );
    }
  }
