...
export class LayoutsManagerComponent implements OnInit {
  ...
  public downloadFile(url) {
    let name = url;
    try {
      //z extract filename rom url (last part without parameters)
      name = /.*\/([^?]+)/.exec(url)[1];
    } catch (e) {}
    ...
    this._layoutService.savePictureLink(name, url).subscribe(
      ...
      (err) => {
        this.meta.notification.show = true;
        this.meta.notification.msg = this.tr('layoutManager.err.linkProblem', {
          url,
        });
      }
    );
  ...
  public loadImgAsBase64(item) {
    ...
    this._layoutService.loadImgAsBase64(item.url).subscribe(
      (image) => {
  ...
  public saveItem(item) {
    ...
    this._layoutService.updatePicture(item).subscribe(() => {
      this.dataReload();
    });
  }
